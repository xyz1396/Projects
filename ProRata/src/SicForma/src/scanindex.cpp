...
bool ScanIndex::BuildIndex(const string fn) {
  ...
  while(!scan_file_stream.eof()) {
    ...
    if(line[0] == 'S') {
      ...
      scan_numbers_.push_back(scan_number);
// cout << "S\t" << scan_number << "\t" << file_position << endl;
    }
    file_position += scan_file_stream.gcount();
// In Windows, gcount will under-count getline because of /r
// Incrementing file_position by 1 to correct this error in Windows 
#if _WIN32
    file_position += 1;
#endif
  }
