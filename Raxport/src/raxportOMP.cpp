...
// https://github.com/thermofisherlsms/lua-raw-file/blob/master/inc/RawFile.h
// wrap IXRawfile5Ptr to make it run in parallel for
typedef struct RawFile
{
	...
	{
		// initialize COM in single thread
		CoInitialize(NULL);
		// Try to create an instance of the preferred Rawfile
		HRESULT result = comRawFile.CreateInstance(MSFR_XRAWFILE);
		if (FAILED(result))
		{
			cout << endl;
			cout << "Error: cannot open MSFileReader XRawFile2.dll" << endl;
			cout << "Please install MSFileReader and Foundation from Thermo Scientific freely available at" << endl;
			cout << "https://thermo.flexnetoperations.com/" << endl;
			exit(0);
		}
		init = result;
		IsOpen = false;
...
// peak data for scan without label data ion trap for example
...
#pragma omp parallel for
	for (int i = 0; i < (int)vsRawFiles.size(); i++)
	{
		...
		RawFile *rawFile = new RawFile(sRawFilename.c_str());
		// Get the initialization error if present
		HRESULT hr = rawFile->comRawFile->Open(rawFile->FileName);
		if (FAILED(hr))
		{
			cout << "Unable to open .RAW file: " << sRawFilename.c_str() << endl;
		}
		rawFile->comRawFile->SetCurrentController(0, 1);
		...
		rawFile->comRawFile->GetInstModel(&bstrInstModel);
		...
		for (long scanNum = firstScanNumber; scanNum <= lastScanNumber; scanNum++)
		{
			...
			*/
			...
			// get MS Order
			long MSOrder = 0;
			rawFile->comRawFile->GetMSOrderForScanNum(scanNum, &MSOrder);
			// cout << "MSOrder = " << MSOrder << endl;

			// get retention time
			double retentionTime = 0;
			rawFile->comRawFile->RTFromScanNum(scanNum, &retentionTime);
			// cout << "retentionTime = " << retentionTime << endl;

			// get the filter text
			BSTR bstrFilter = NULL;
			rawFile->comRawFile->GetFilterForScanNum(scanNum, &bstrFilter);
			...
			rawFile->comRawFile->GetMassAnalyzerTypeForScanNum(scanNum, &iMassAnalyzerType);
			...
			rawFile->comRawFile->GetScanTypeForScanNum(scanNum, &iScanType);
			switch (iScanType)
			{
			case 0:
				sScanTypeName = "Full";
			...
			if (MSOrder == 2)
			{
				...
				rawFile->comRawFile->GetActivationTypeForScanNum(scanNum, MSOrder, &iActivationType);
				...
				// get isolation width
				//		rawFile->comRawFile->GetIsolationWidthForScanNum(scanNum, MSOrder, &dIsolationWidth);
				...
				rawFile->comRawFile->GetPrecursorInfoFromScanNum(scanNum, &vPrecursorInfos, &nPrecursorInfos);
				...
				rawFile->comRawFile->GetTrailerExtraValueForScanNum(scanNum, "Monoisotopic M/Z:", &varPrecursor);
				...
				rawFile->comRawFile->GetTrailerExtraValueForScanNum(scanNum, "Charge State:", &varCharge);
				...
				rawFile->comRawFile->GetPrecursorMassForScanNum(scanNum, MSOrder, &precursorMZ_IsolationTarget);
			...
			rawFile->comRawFile->GetLabelData(&varLabels, &varFlags, &scanNum);
			...
			if (peakNumber > 0)
			{
				...
				// get peak by label data
				LabelData *pValues = NULL;
				SAFEARRAY FAR *valueSA = varLabels.parray;
				...
				if (MSOrder == 1 && (!noFT1))
				{
					...
					{
						FT1stream
							<< fixed << setprecision(5) << pValues[n].Mass << "\t"
							<< fixed << setprecision(2) << pValues[n].Intensity << "\t"
							<< fixed << setprecision(0) << pValues[n].Resolution << "\t"
							<< fixed << setprecision(2) << pValues[n].Baseline << "\t"
							<< fixed << setprecision(2) << pValues[n].Noise << "\t"
							<< fixed << setprecision(0) << pValues[n].Charge << endl;
					}
				}
				if (MSOrder != 1 && (!noFT2))
				{
			...
			// some scan dose not have label data
			// ion trap scan for example
			else
			{
				// // test the raw file with scan without label data
				// cout << sRawFilename << endl;
				...
				long nArraySize = 0;
				double peakWidth = 0;
				rawFile->comRawFile->GetMassListFromScanNum(&scanNum,
		...
		cout << "\nFinished Convert " << sRawFilename.c_str() << endl;
...
// https://stackoverflow.com/questions/510805/can-a-win32-console-application-detect-if-it-has-been-run-from-the-explorer-or-n
...
// https://www.cnblogs.com/182-7167-2685/p/5067909.html
// get path by dialog
...
static string GetExistingDirectory()
{
	...
	bi.lParam = NULL;
	bi.lpfn = NULL;
	bi.lpszTitle = _T("Please select raw file path");
...
{
	string sPath = "";
	// detect the exe is run from click or console
	if (!separate_console())
	{
		cout << "Run from consle" << endl;
	...
	{
		cout << "Run from click" << endl;
		sPath = GetExistingDirectory();
	}
	...
	{
		bool finished = ProcessFiles(vsRawFilenames);
		if (finished)
			cout << "All Convert finished\n"
				 << endl;
		else
