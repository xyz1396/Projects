...
bool ProteinDatabase::getNextOriginalPeptide(Peptide* originalPeptide)
{
	...
	{
	    if (isPeptideLengthGood(vicleavageSite.at(iendCleavagePos)
	      -vicleavageSite.at(ibeginCleavagePos)))
	    {
		sOriginalPeptide = "[" + scurrentProtein
		    .substr(vicleavageSite.at(ibeginCleavagePos)+1,
			vicleavageSite.at(iendCleavagePos)
			-vicleavageSite.at(ibeginCleavagePos))+ "]";
		dOriginalPeptideMass = msum_map[vicleavageSite.at(iendCleavagePos)]
				      + ProNovoConfig::getTerminusMassN()
...
//	    cout<<ptm_position_all.at(comb_order.at(i)).second.size()<<endl;
	    sPtm    = ptm_position_all.at(comb_order.at(i))
			.second.at(ptm_order.at(i)).first;

	    dPtmMass= ptm_position_all.at(comb_order.at(i))
			.second.at(ptm_order.at(i)).second;
	...
	const string& sOriginalPeptideContent, int iendCleavagePos, string& sMutatedPeptide)
{
	...
	if ((imutationPos == 0) && (ibeginCleavagePos != 0)) 
	    ...
	    if ((imutationPos == ((int)sOriginalPeptideContent.length()-1)) 
...
// Mutation can create a new cleavage site. So we have to consider new peptides
{
	...
	if (bRightSubpeptide)
	{
	    ...
	    if (imutationPos < (int)sOriginalPeptideContent.length())
	    {  
		if (iclCheck < ProNovoConfig::getMaxMissedCleavages())
